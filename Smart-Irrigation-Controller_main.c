...
int calculatePWM(){
	...
	HAL_ADC_PollForConversion(&hadc1, 1000);
	uint8_t ADC_CH9 = HAL_ADC_GetValue(&hadc1);
	return (((ADC_CH9 << 6) + (ADC_CH9 << 5) + (ADC_CH9 << 2)) >> 8 );
}
...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  sprintf((char*)txd_msg_buffer10, "\r\nTIME: %d INLET MODE -- PWM: %d -- RPM: %d DEPTH: %d", clock_hours,  zones[0] == 0 ? calculatePWM() : zones[0], (int) rpm, distance);
	  ...
	  while (distance <= 650 && distance != 0)
	  {
		  ...
		  while ((time[3] <= clock_hours && clock_hours <= time[4]) && temp == 0) {
			  ...
			  // Terminal Display
			  if(clock_hours != pastHour){
				  sprintf((char*)txd_msg_buffer12, "\r\nTIME: %d ZONE 1 -- PWM: %d -- RPM: %d DEPTH: %d", clock_hours,  zones[1] == 0 ? calculatePWM() : zones[1], (int) rpm, distance);
...
//			  }
		  ...
		  while ((time[5] <= clock_hours && clock_hours <= time[6])&& temp == 0)
		  {
			  ...
			  // Terminal Display
			  if(clock_hours != pastHour) {
				  sprintf((char*)txd_msg_buffer13, "\r\nTIME: %d ZONE 2 -- PWM: %d -- RPM: %d DEPTH: %d", clock_hours,  zones[2] == 0 ? calculatePWM() : zones[2], (int) rpm, distance);
...
//			  }
		  ...
		  // dc motor runs for user specified time
		  while ((time[7] <= clock_hours && clock_hours <= time[8] && temp == 0) && temp == 0)
		  {
			  temp = 0;
			  // Timer
			  TIM3->PSC = 160-1;
			  ...
			  // Terminal Display
			  if(clock_hours != pastHour){
				  sprintf((char*)txd_msg_buffer14, "\r\nTIME: %d ZONE 3 -- PWM: %d -- RPM: %d DEPTH: %d", clock_hours,  zones[3] == 0 ? calculatePWM() : zones[3], (int) rpm, distance);
